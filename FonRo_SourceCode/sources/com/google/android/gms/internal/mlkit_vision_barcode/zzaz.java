...
public final class zzaz {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            zzb = zzba.zzb("%s (%s) must not be negative", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException("negative size: " + i2);
            }
            zzb = zzba.zzb("%s (%s) must be less than size (%s)", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static void zze(int i, int i2, int i3) {
        ...
        if (i < 0 || i2 < i || i2 > i3) {
            if (i < 0 || i > i3) {
                zzg = zzg(i, i3, "start index");
            } else if (i2 < 0 || i2 > i3) {
                zzg = zzg(i2, i3, "end index");
            } else {
                zzg = zzba.zzb("end index (%s) must not be less than start index (%s)", Integer.valueOf(i2), Integer.valueOf(i));
            }
