...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo1002calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        if (this.width == IntrinsicSize.Min) {
            maxIntrinsicWidth = measurable.minIntrinsicWidth(Constraints.m6941getMaxHeightimpl(j));
        } else {
            maxIntrinsicWidth = measurable.maxIntrinsicWidth(Constraints.m6941getMaxHeightimpl(j));
        }
        if (maxIntrinsicWidth < 0) {
            maxIntrinsicWidth = 0;
        }
        return Constraints.INSTANCE.m6954fixedWidthOenEA2s(maxIntrinsicWidth);
    }
