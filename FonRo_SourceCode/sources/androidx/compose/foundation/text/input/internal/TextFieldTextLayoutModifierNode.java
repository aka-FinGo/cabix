...
public final class TextFieldTextLayoutModifierNode extends Modifier.Node implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo384measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult m1625layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1625layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable mo5732measureBRTryo0 = measurable.mo5732measureBRTryo0(Constraints.INSTANCE.m6951fitPrioritizingWidthZbe2FdA((int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() >> 32), (int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() >> 32), (int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L), (int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L)));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            m6989constructorimpl = measureScope.mo697toDpu2uoSUM(TextDelegateKt.ceilToIntPx(m1625layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            m6989constructorimpl = Dp.m6989constructorimpl(0.0f);
        }
        textLayoutState.m1626setMinHeightForSingleLineField0680j_4(m6989constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(m1625layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(m1625layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int size = (int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() >> 32);
        int size2 = (int) (m1625layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L);
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout(size, size2, map, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$measure$1
