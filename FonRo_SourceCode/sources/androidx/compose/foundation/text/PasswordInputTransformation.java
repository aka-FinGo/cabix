...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m6431getLengthimpl(textFieldBuffer.getChanges().mo1509getRangejx7JFs(0)) != 1 || TextRange.m6431getLengthimpl(textFieldBuffer.getChanges().mo1508getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int m6433getMinimpl = TextRange.m6433getMinimpl(textFieldBuffer.getChanges().mo1509getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != m6433getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(m6433getMinimpl);
        }
    }
