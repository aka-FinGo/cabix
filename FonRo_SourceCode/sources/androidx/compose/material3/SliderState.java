...
public final class SliderState implements DraggableState {
    ...
    public void dispatchRawDelta(float delta) {
        float snapValueToTick;
        float max = Math.max(getTotalWidth() - (getThumbWidth$material3_release() / 2.0f), 0.0f);
        float min = Math.min(getThumbWidth$material3_release() / 2.0f, max);
    ...
    public final void m2630onPressk4lQ0M$material3_release(long pos) {
        setPressOffset((this.isRtl ? getTotalWidth() - Offset.m4125getXimpl(pos) : Offset.m4125getXimpl(pos)) - getRawOffset());
    }
