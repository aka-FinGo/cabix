...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int i;
        this.rippleSize = contentDrawScope.mo4935getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1845getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo4935getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo694roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m4394unboximpl = this.color.getValue().m4394unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1849drawStateLayerH2RKhps(contentDrawScope2, this.radius, m4394unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1848setRipplePropertiesbiQXAtU(contentDrawScope.mo4935getSizeNHjbRc(), this.rippleRadius, m4394unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1847addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m4394unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
