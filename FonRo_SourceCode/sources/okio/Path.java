...
public final class Path implements Comparable<Path> {
    ...
    public final Path relativeTo(Path r9) {
        Intrinsics.checkNotNullParameter(r9, "other");
        if (!Intrinsics.areEqual(getRoot(), r9.getRoot())) {
            throw new IllegalArgumentException(("Paths of different roots cannot be relative to each other: " + this + " and " + r9).toString());
        }
        List<ByteString> segmentsBytes = getSegmentsBytes();
        List<ByteString> segmentsBytes2 = r9.getSegmentsBytes();
        int min = Math.min(segmentsBytes.size(), segmentsBytes2.size());
        int i = 0;
        while (i < min && Intrinsics.areEqual(segmentsBytes.get(i), segmentsBytes2.get(i))) {
            i++;
        }
        if (i != min || getBytes().size() != r9.getBytes().size()) {
            if (segmentsBytes2.subList(i, segmentsBytes2.size()).indexOf(okio.internal.Path.DOT_DOT) != -1) {
                throw new IllegalArgumentException(("Impossible relative path to resolve: " + this + " and " + r9).toString());
            }
            Buffer buffer = new Buffer();
            ByteString slash = okio.internal.Path.getSlash(r9);
    ...
    public int compareTo(Path r2) {
        Intrinsics.checkNotNullParameter(r2, "other");
        return getBytes().compareTo(r2.getBytes());
    }

    public boolean equals(Object r2) {
        return (r2 instanceof Path) && Intrinsics.areEqual(((Path) r2).getBytes(), getBytes());
    }
