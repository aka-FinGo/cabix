...
public final class TextFieldCoreModifierNode extends DelegatingNode implements LayoutModifierNode, DrawModifierNode, CompositionLocalConsumerModifierNode, GlobalPositionAwareModifierNode, SemanticsModifierNode {
    ...
    public MeasureResult mo384measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        if (this.orientation == Orientation.Vertical) {
            return m1604measureVerticalScroll3p2s80s(measureScope, measurable, j);
        }
        return m1603measureHorizontalScroll3p2s80s(measureScope, measurable, j);
    }
    ...
    public void draw(ContentDrawScope contentDrawScope) {
        ...
        if (TextRange.m6429getCollapsedimpl(visualText.getSelection())) {
            ...
            if (visualText.shouldShowSelection()) {
                m1602drawSelectionSbBc2M(contentDrawScope, visualText.getSelection(), layoutResult);
            }
    ...
    private final MeasureResult m1604measureVerticalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable mo5732measureBRTryo0 = measurable.mo5732measureBRTryo0(Constraints.m6932copyZbe2FdA$default(j, 0, 0, 0, Integer.MAX_VALUE, 7, null));
        final int min = Math.min(mo5732measureBRTryo0.getHeight(), Constraints.m6941getMaxHeightimpl(j));
        return MeasureScope.layout$default(measureScope, mo5732measureBRTryo0.getWidth(), min, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureVerticalScroll$1
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                ...
                textFieldCoreModifierNode.m1605updateScrollStatetIlFzwE(measureScope2, i, height, transformedTextFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable placeable = mo5732measureBRTryo0;
    ...
    private final MeasureResult m1603measureHorizontalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable mo5732measureBRTryo0 = measurable.mo5732measureBRTryo0(Constraints.m6932copyZbe2FdA$default(j, 0, Integer.MAX_VALUE, 0, 0, 13, null));
        final int min = Math.min(mo5732measureBRTryo0.getWidth(), Constraints.m6942getMaxWidthimpl(j));
        return MeasureScope.layout$default(measureScope, min, mo5732measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureHorizontalScroll$1
            /* JADX INFO: Access modifiers changed from: package-private */
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                ...
                transformedTextFieldState = TextFieldCoreModifierNode.this.textFieldState;
                textFieldCoreModifierNode.m1605updateScrollStatetIlFzwE(measureScope2, i, width, transformedTextFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable placeable = mo5732measureBRTryo0;
                scrollState = TextFieldCoreModifierNode.this.scrollState;
    ...
    private final int m1601calculateOffsetToFollow72CqOWE(long currSelection, int currTextLayoutSize) {
        TextRange textRange = this.previousSelection;
        if (textRange == null || TextRange.m6430getEndimpl(currSelection) != TextRange.m6430getEndimpl(textRange.getPackedValue())) {
            return TextRange.m6430getEndimpl(currSelection);
        }
        TextRange textRange2 = this.previousSelection;
        if (textRange2 == null || TextRange.m6435getStartimpl(currSelection) != TextRange.m6435getStartimpl(textRange2.getPackedValue())) {
            return TextRange.m6435getStartimpl(currSelection);
        }
    ...
    public final void m1605updateScrollStatetIlFzwE(Density density, int i, int i2, long j, LayoutDirection layoutDirection) {
        TextLayoutResult layoutResult;
        Rect cursorRectInScroller;
        float f;
        this.scrollState.setMaxValue$foundation_release(i2 - i);
        int m1601calculateOffsetToFollow72CqOWE = m1601calculateOffsetToFollow72CqOWE(j, i2);
        if (m1601calculateOffsetToFollow72CqOWE < 0 || !getShowCursor() || (layoutResult = this.textLayoutState.getLayoutResult()) == null) {
            return;
        }
        Rect cursorRect = layoutResult.getCursorRect(RangesKt.coerceIn(m1601calculateOffsetToFollow72CqOWE, (ClosedRange<Integer>) new IntRange(0, layoutResult.getLayoutInput().getText().length())));
        ...
        if (bottom <= f2) {
            ...
            if (top >= f3 || bottom - top <= i) {
                ...
                this.previousSelection = TextRange.m6423boximpl(j);
    ...
    private final void m1602drawSelectionSbBc2M(DrawScope drawScope, long j, TextLayoutResult textLayoutResult) {
        int m6433getMinimpl = TextRange.m6433getMinimpl(j);
        int m6432getMaximpl = TextRange.m6432getMaximpl(j);
        if (m6433getMinimpl != m6432getMaximpl) {
            DrawScope.m4925drawPathLG529CI$default(drawScope, textLayoutResult.getPathForRange(m6433getMinimpl, m6432getMaximpl), ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
        }
    }
    ...
    private final void drawHighlight(DrawScope drawScope, Pair<TextHighlightType, TextRange> pair, TextLayoutResult textLayoutResult) {
        ...
        if (TextRange.m6429getCollapsedimpl(packedValue)) {
            return;
        }
        Path pathForRange = textLayoutResult.getPathForRange(TextRange.m6433getMinimpl(packedValue), TextRange.m6432getMaximpl(packedValue));
        if (TextHighlightType.m1521equalsimpl0(value, TextHighlightType.INSTANCE.m1525getHandwritingDeletePreviewsxJuwY())) {
            Brush brush = textLayoutResult.getLayoutInput().getStyle().getBrush();
            if (brush != null) {
                DrawScope.m4924drawPathGBMwjPU$default(drawScope, pathForRange, brush, 0.2f, null, null, 0, 56, null);
                return;
            }
            long m6465getColor0d7_KjU = textLayoutResult.getLayoutInput().getStyle().m6465getColor0d7_KjU();
            if (m6465getColor0d7_KjU == 16) {
                m6465getColor0d7_KjU = Color.INSTANCE.m4410getBlack0d7_KjU();
            }
            long j = m6465getColor0d7_KjU;
            DrawScope.m4925drawPathLG529CI$default(drawScope, pathForRange, Color.m4383copywmQWz5c$default(j, Color.m4386getAlphaimpl(j) * 0.2f, 0.0f, 0.0f, 0.0f, 14, null), 0.0f, null, null, 0, 60, null);
            return;
        }
        DrawScope.m4925drawPathLG529CI$default(drawScope, pathForRange, ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
    }
    ...
    private final void drawCursor(DrawScope drawScope) {
        ...
        if (cursorAlpha != 0.0f && getShowCursor()) {
            ...
            DrawScope.m4920drawLine1RTmtNc$default(drawScope, this.cursorBrush, cursorRect.m4159getTopCenterF1C5BW0(), cursorRect.m4152getBottomCenterF1C5BW0(), cursorRect.getRight() - cursorRect.getLeft(), 0, null, cursorAlpha, null, 0, 432, null);
        }
    }
