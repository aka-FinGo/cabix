...
public final class FlowLayoutBuildingBlocks {
    ...
    /* compiled from: FlowLayoutBuildingBlocks.kt */
    @Metadata(d1 = {"\u0000\u0012\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\u000b\n\u0002\b\u0004\b\u0007\u0018\u00002\u00020\u0001B\u0019\u0012\b\b\u0002\u0010\u0002\u001a\u00020\u0003\u0012\b\b\u0002\u0010\u0004\u001a\u00020\u0003¢\u0006\u0002\u0010\u0005R\u0011\u0010\u0004\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0004\u0010\u0006R\u0011\u0010\u0002\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0002\u0010\u0006¨\u0006\u0007"}, d2 = {"Landroidx/compose/foundation/layout/FlowLayoutBuildingBlocks$WrapInfo;", "", "isLastItemInLine", "", "isLastItemInContainer", "(ZZ)V", "()Z", "foundation-layout_release"}, k = 1, mv = {1, 9, 0}, xi = ConstraintLayout.LayoutParams.Table.LAYOUT_CONSTRAINT_VERTICAL_CHAINSTYLE)
    /* loaded from: classes.dex */
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        ellipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m344getFirstimpl(ellipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
        return ellipsisInfo$foundation_layout_release;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m971getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout_release() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m345getSecondimpl(leftOver) - IntIntPair.m345getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m344getFirstimpl(leftOver) - IntIntPair.m344getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m971getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m340constructorimpl(Constraints.m6942getMaxWidthimpl(this.constraints), (IntIntPair.m345getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m337boximpl(IntIntPair.m340constructorimpl(IntIntPair.m344getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m345getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int max = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m345getSecondimpl(nextSize.getPackedValue()));
        IntIntPair m976ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m976ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, max);
        if (m976ellipsisSizeF35zmw$foundation_layout_release != null) {
            m976ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m344getFirstimpl(leftOver) - IntIntPair.m344getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m344getFirstimpl(m976ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo m971getWrapInfoOpUlnko = m971getWrapInfoOpUlnko(false, 0, IntIntPair.m340constructorimpl(Constraints.m6942getMaxWidthimpl(this.constraints), (IntIntPair.m345getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m345getSecondimpl(nextSize.getPackedValue()))), m976ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, max, 0, true, true);
                return new WrapInfo(m971getWrapInfoOpUlnko.getIsLastItemInContainer(), m971getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
