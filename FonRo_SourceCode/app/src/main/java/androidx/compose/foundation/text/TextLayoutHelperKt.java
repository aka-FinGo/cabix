...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m1485canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), annotatedString) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m6915equalsimpl0(layoutInput.getOverflow(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) || Constraints.m6944getMinWidthimpl(j) != Constraints.m6944getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m6915equalsimpl0(i2, TextOverflow.INSTANCE.m6925getEllipsisgIe3tQ8())) {
            return Constraints.m6942getMaxWidthimpl(j) == Constraints.m6942getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m6941getMaxHeightimpl(j) == Constraints.m6941getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }

    /* renamed from: isPositionInsideSelection-uaM50fQ, reason: not valid java name */
    public static final boolean m1486isPositionInsideSelectionuaM50fQ(TextLayoutResult textLayoutResult, long j, TextRange textRange) {
        if (textRange == null || TextRange.m6429getCollapsedimpl(textRange.getPackedValue())) {
            return false;
        }
        int m6405getOffsetForPositionk4lQ0M = textLayoutResult.m6405getOffsetForPositionk4lQ0M(j);
        return isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m6405getOffsetForPositionk4lQ0M) || isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m6405getOffsetForPositionk4lQ0M - 1);
    }
    ...
    private static final boolean isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(TextRange textRange, TextLayoutResult textLayoutResult, long j, int i) {
        return TextRange.m6426containsimpl(textRange.getPackedValue(), i) && textLayoutResult.getBoundingBox(i).m4151containsk4lQ0M(j);
    }
