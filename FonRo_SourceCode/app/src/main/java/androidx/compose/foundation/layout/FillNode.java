...
final class FillNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo384measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int i;
        int i2;
        if (!Constraints.m6938getHasBoundedWidthimpl(j) || this.direction == Direction.Vertical) {
            m6944getMinWidthimpl = Constraints.m6944getMinWidthimpl(j);
            m6942getMaxWidthimpl = Constraints.m6942getMaxWidthimpl(j);
        } else {
            int round = Math.round(Constraints.m6942getMaxWidthimpl(j) * this.fraction);
            int m6944getMinWidthimpl2 = Constraints.m6944getMinWidthimpl(j);
            m6944getMinWidthimpl = Constraints.m6942getMaxWidthimpl(j);
            if (round < m6944getMinWidthimpl2) {
                round = m6944getMinWidthimpl2;
            }
            if (round <= m6944getMinWidthimpl) {
                m6944getMinWidthimpl = round;
            }
            m6942getMaxWidthimpl = m6944getMinWidthimpl;
        }
        if (!Constraints.m6937getHasBoundedHeightimpl(j) || this.direction == Direction.Horizontal) {
            ...
            i2 = m6941getMaxHeightimpl;
        } else {
            int round2 = Math.round(Constraints.m6941getMaxHeightimpl(j) * this.fraction);
            int m6943getMinHeightimpl2 = Constraints.m6943getMinHeightimpl(j);
            i = Constraints.m6941getMaxHeightimpl(j);
            if (round2 < m6943getMinHeightimpl2) {
                round2 = m6943getMinHeightimpl2;
            }
            if (round2 <= i) {
                i = round2;
            }
        ...
        final Placeable mo5732measureBRTryo0 = measurable.mo5732measureBRTryo0(ConstraintsKt.Constraints(m6944getMinWidthimpl, m6942getMaxWidthimpl, i, i2));
        return MeasureScope.layout$default(measureScope, mo5732measureBRTryo0.getWidth(), mo5732measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.FillNode$measure$1
