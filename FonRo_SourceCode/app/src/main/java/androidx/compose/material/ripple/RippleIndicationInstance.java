...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m1849drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float f2;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            f2 = RippleAnimationKt.m1845getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo4935getSizeNHjbRc());
        } else {
            f2 = drawScope.mo700toPx0680j_4(f);
        }
        stateLayer.m1857drawStateLayermxwnekA(drawScope, f2, j);
    }
}
