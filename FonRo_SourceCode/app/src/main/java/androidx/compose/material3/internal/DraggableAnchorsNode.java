...
public final class DraggableAnchorsNode<T> extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo384measure3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        ...
        if (!measureScope.isLookingAhead() || !this.didLookahead) {
            Pair<? extends DraggableAnchors<T>, ? extends T> invoke = this.anchors.invoke(IntSize.m7152boximpl(IntSizeKt.IntSize(mo5732measureBRTryo0.getWidth(), mo5732measureBRTryo0.getHeight())), Constraints.m6929boximpl(j));
            this.state.updateAnchors(invoke.getFirst(), invoke.getSecond());
        }
        this.didLookahead = measureScope.isLookingAhead() || this.didLookahead;
        return MeasureScope.layout$default(measureScope, mo5732measureBRTryo0.getWidth(), mo5732measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.material3.internal.DraggableAnchorsNode$measure$1
            /* JADX INFO: Access modifiers changed from: package-private */
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                ...
                if (isLookingAhead) {
                    requireOffset = draggableAnchorsNode.getState().getAnchors().positionOf(this.getState().getTargetValue());
                } else {
                    requireOffset = draggableAnchorsNode.getState().requireOffset();
                }
                float f = this.getOrientation() == Orientation.Horizontal ? requireOffset : 0.0f;
                if (this.getOrientation() != Orientation.Vertical) {
                    requireOffset = 0.0f;
                }
                Placeable.PlacementScope.place$default(placementScope, mo5732measureBRTryo0, MathKt.roundToInt(f), MathKt.roundToInt(requireOffset), 0.0f, 4, null);
            }
        }, 4, null);
    }
}
