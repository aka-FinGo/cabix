...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo4221concat58bKbWc(float[] matrix) {
        if (MatrixKt.m4639isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m4236setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo4220clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m4229toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo4219clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m4229toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m4229toRegionOp7u2Bmg(int i) {
        return ClipOp.m4368equalsimpl0(i, ClipOp.INSTANCE.m4372getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo4225drawLineWko1d7g(long p1, long p2, Paint paint) {
    ...
    public void mo4222drawCircle9KIMszo(long center, float radius, Paint paint) {
    ...
    public void mo4223drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
    ...
    public void mo4224drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m7117getXimpl(srcOffset);
        rect.top = IntOffset.m7118getYimpl(srcOffset);
        rect.right = IntOffset.m7117getXimpl(srcOffset) + ((int) (srcSize >> 32));
        rect.bottom = IntOffset.m7118getYimpl(srcOffset) + ((int) (srcSize & 4294967295L));
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m7117getXimpl(dstOffset);
        rect2.top = IntOffset.m7118getYimpl(dstOffset);
        rect2.right = IntOffset.m7117getXimpl(dstOffset) + ((int) (dstSize >> 32));
        rect2.bottom = IntOffset.m7118getYimpl(dstOffset) + ((int) (dstSize & 4294967295L));
    ...
    public void mo4226drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m4685equalsimpl0(pointMode, PointMode.INSTANCE.m4689getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m4685equalsimpl0(pointMode, PointMode.INSTANCE.m4691getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m4685equalsimpl0(pointMode, PointMode.INSTANCE.m4690getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                long m4135unboximpl = points.get(i).m4135unboximpl();
                long m4135unboximpl2 = points.get(i + 1).m4135unboximpl();
                this.internalCanvas.drawLine(Float.intBitsToFloat((int) (m4135unboximpl >> 32)), Float.intBitsToFloat((int) (m4135unboximpl & 4294967295L)), Float.intBitsToFloat((int) (m4135unboximpl2 >> 32)), Float.intBitsToFloat((int) (m4135unboximpl2 & 4294967295L)), asFrameworkPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo4227drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo4228drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m4272toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.asFrameworkPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long m4135unboximpl = points.get(i).m4135unboximpl();
            this.internalCanvas.drawPoint(Float.intBitsToFloat((int) (m4135unboximpl >> 32)), Float.intBitsToFloat((int) (m4135unboximpl & 4294967295L)), paint.asFrameworkPaint());
        }
    }
}
