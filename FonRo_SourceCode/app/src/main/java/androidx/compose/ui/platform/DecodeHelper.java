...
public final class DecodeHelper {
        ...
        while (this.parcel.dataAvail() > 1) {
            ...
            if (decodeByte != 1) {
                if (decodeByte != 2) {
                    if (decodeByte != 3) {
                        if (decodeByte != 4) {
                            if (decodeByte != 5) {
                                        ...
                                        if (decodeByte != 8) {
                                            if (decodeByte != 9) {
                                                if (decodeByte != 10) {
                                                    ...
                                                    mutableSpanStyle.m6159setBackground8_81llA(m6124decodeColor0d7_KjU());
                                                }
                                            } else {
                                                if (dataAvailable() < 8) {
                                                    break;
                                            ...
                                            mutableSpanStyle.m6160setBaselineShift_isdbwI(BaselineShift.m6745boximpl(m6122decodeBaselineShifty9eOQZs()));
                                        }
                                    } else {
                                        if (dataAvailable() < 5) {
                                            break;
                                        }
                                        mutableSpanStyle.m6165setLetterSpacingR2X_6o(m6127decodeTextUnitXSAIIZE());
                                    }
                                } else {
                                ...
                                mutableSpanStyle.m6164setFontSynthesistDdu0R4(FontSynthesis.m6574boximpl(m6126decodeFontSynthesisGVVA2EU()));
                            }
                        } else {
                            if (dataAvailable() < 1) {
                                break;
                            }
                            mutableSpanStyle.m6163setFontStylemLjRB2g(FontStyle.m6563boximpl(m6125decodeFontStyle_LCdwA()));
                        }
                    } else {
                        if (dataAvailable() < 4) {
                            break;
                    ...
                    mutableSpanStyle.m6162setFontSizeR2X_6o(m6127decodeTextUnitXSAIIZE());
                }
            } else {
                if (dataAvailable() < 8) {
                    break;
                }
                mutableSpanStyle.m6161setColor8_81llA(m6124decodeColor0d7_KjU());
            }
        }
    ...
    public final long m6124decodeColor0d7_KjU() {
        return Color.m4380constructorimpl(m6123decodeULongsVKNKU());
    }

    /* renamed from: decodeTextUnit-XSAIIZE, reason: not valid java name */
    public final long m6127decodeTextUnitXSAIIZE() {
    ...
    public final int m6125decodeFontStyle_LCdwA() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    public final int m6126decodeFontSynthesisGVVA2EU() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    private final float m6122decodeBaselineShifty9eOQZs() {
    ...
    private final Shadow decodeShadow() {
        long m6124decodeColor0d7_KjU = m6124decodeColor0d7_KjU();
        float decodeFloat = decodeFloat();
        float decodeFloat2 = decodeFloat();
        return new Shadow(m6124decodeColor0d7_KjU, Offset.m4117constructorimpl((Float.floatToRawIntBits(decodeFloat2) & 4294967295L) | (Float.floatToRawIntBits(decodeFloat) << 32)), decodeFloat(), null);
    }
    ...
    private final long m6123decodeULongsVKNKU() {
