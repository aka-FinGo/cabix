...
public abstract class HoverIconModifierNode extends Modifier.Node implements TraversableNode, PointerInputModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public abstract boolean mo5534isRelevantPointerTypeuerMTgs(int pointerType);
    ...
    public void mo528onPointerEventH0pRuoY(PointerEvent pointerEvent, PointerEventPass pass, long bounds) {
        if (pass == PointerEventPass.Main) {
            ...
            for (int i = 0; i < size; i++) {
                if (mo5534isRelevantPointerTypeuerMTgs(changes.get(i).getType())) {
                    if (PointerEventType.m5556equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m5560getEnter7fucELk())) {
                        onEnter();
                        return;
                    } else {
                        if (PointerEventType.m5556equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m5561getExit7fucELk())) {
                            onExit();
                            return;
                        }
                        return;
    ...
    public long mo1498getTouchBoundsExpansionRZrCHBk() {
        DpTouchBoundsExpansion dpTouchBoundsExpansion = this.dpTouchBoundsExpansion;
        return dpTouchBoundsExpansion != null ? dpTouchBoundsExpansion.m5890roundToTouchBoundsExpansionTW6G1oQ(DelegatableNodeKt.requireDensity(this)) : TouchBoundsExpansion.INSTANCE.m6089getNoneRZrCHBk();
    }
