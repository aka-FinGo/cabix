...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6433getMinimpl(j);
        this.selectionEnd = TextRange.m6432getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m6433getMinimpl = TextRange.m6433getMinimpl(j);
        int m6432getMaximpl = TextRange.m6432getMaximpl(j);
        if (m6433getMinimpl < 0 || m6433getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m6433getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6432getMaximpl < 0 || m6432getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m6432getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6433getMinimpl <= m6432getMaximpl) {
            return;
        }
        throw new IllegalArgumentException("Do not set reversed range: " + m6433getMinimpl + " > " + m6432getMaximpl);
    }
    ...
    public final TextRange m6615getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m6423boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m6616getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long m6617updateRangeAfterDeletepWDy79M = EditingBufferKt.m6617updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m6433getMinimpl(m6617updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6432getMaximpl(m6617updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m6617updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m6617updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m6429getCollapsedimpl(m6617updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m6433getMinimpl(m6617updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6432getMaximpl(m6617updateRangeAfterDeletepWDy79M2);
            }
        }
    }
